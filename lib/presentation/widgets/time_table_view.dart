  ...
  Widget build(BuildContext context) {
    return BlocBuilder<TimeTableViewCubit, TimeTableViewState>(
      ...
      builder: (context, state) {
        return switch (state) {
          TimeTableViewInitial() => const Center(child: CircularProgressIndicator()),
...
class _TimeTableContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (slot.type == "PR" || slot.type == "TT") {
      final subSlot = (slot.type == "PR") ? slot.subSlots![batchIndex] : slot.subSlots![groupIndex];

      slotTitle = subSlot.subject ?? subSlot.activity ?? '';
      slotTypeText = (slot.type == "PR") ? 'Batch: ${subSlot.batch ?? 'N/A'}' : 'Group: ${subSlot.group ?? 'N/A'}';
    }
        ...
        children: [
          ...
          Row(
            ...
            children: [
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Text(
                      slotTypeText,
                      style: TextStyle(
                        fontFamily: 'NovaFlat', // Ensure the font is added in pubspec.yaml
                        color: Colors.deepPurple[100],
              ...
              AnimatedButton(
                  index: currentIndex,
                  onTap: () => context.read<TimeTableViewCubit>().previousSlot(),
...
class TimeTableContainer extends StatelessWidget {
  const TimeTableContainer({super.key, required this.mainContent, required this.bottomContent, this.onSwipe});
...
class AnimatedButton extends StatefulWidget {
  const AnimatedButton({super.key, this.onTap, required this.index, required this.icon, required this.disableLogic});
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedScale(
        ...
        child: HugeIcon(
          icon: widget.icon,
          color: widget.disableLogic ? Colors.deepPurple[300]! : Colors.deepPurple[100]!,
          size: 32.0,
        ),
      ),
    );
